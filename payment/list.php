...
<nav aria-label="Page navigation" class="mb-4">
    <ul class="pagination justify-content-center" style="gap: 0.5rem;">
        ...
        <li class="page-item <?= $page === 1 ? 'disabled' : '' ?>">
            <a class="page-link" href="/payment/list.php?page=<?= max(1, $page - 1) ?>&q=<?= htmlspecialchars($_GET['q'] ?? '') ?>&from=<?= htmlspecialchars($_GET['from'] ?? '') ?>&to=<?= htmlspecialchars($_GET['to'] ?? '') ?>" style="border-radius: 6px; border: none; background-color: #f5f5f5; color: #667eea;">
        ...
        if ($startPage > 1) {
            echo '<li class="page-item"><a class="page-link" href="/payment/list.php?page=1" style="border-radius: 6px; border: none; background-color: #f5f5f5; color: #667eea;">1</a></li>';
            if ($startPage > 2) {
        ...
        for ($p = $startPage; $p <= $endPage; $p++) {
            if ($p === $page) {
                ...
                echo '<li class="page-item"><a class="page-link" href="/payment/list.php?page=' . $p . '&q=' . htmlspecialchars($_GET['q'] ?? '') . '&from=' . htmlspecialchars($_GET['from'] ?? '') . '&to=' . htmlspecialchars($_GET['to'] ?? '') . '" style="border-radius: 6px; border: none; background-color: #f5f5f5; color: #667eea;">' . $p . '</a></li>';
            }
        }
        ...
        if ($endPage < $maxPages) {
            ...
            echo '<li class="page-item"><a class="page-link" href="/payment/list.php?page=' . $maxPages . '&q=' . htmlspecialchars($_GET['q'] ?? '') . '&from=' . htmlspecialchars($_GET['from'] ?? '') . '&to=' . htmlspecialchars($_GET['to'] ?? '') . '" style="border-radius: 6px; border: none; background-color: #f5f5f5; color: #667eea;">' . $maxPages . '</a></li>';
        }
        ?>

        <!-- Next Button -->
        <li class="page-item <?= $page === $maxPages ? 'disabled' : '' ?>">
            <a class="page-link" href="/payment/list.php?page=<?= min($maxPages, $page + 1) ?>&q=<?= htmlspecialchars($_GET['q'] ?? '') ?>&from=<?= htmlspecialchars($_GET['from'] ?? '') ?>&to=<?= htmlspecialchars($_GET['to'] ?? '') ?>" style="border-radius: 6px; border: none; background-color: #f5f5f5; color: #667eea;">
